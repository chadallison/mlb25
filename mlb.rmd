...
get_game_pitcher_outs_bases = function(pk) {
  pitcher_outs_recorded = mlb_pbp(game_pk = pk) |>
  ...
  pitcher_bases_allowed = mlb_pbp(game_pk = pk) |>
    filter(!is.na(pitchNumber) & result.eventType != "field_error") |>
    group_by(atBatIndex) |>
    slice_max(index, n = 1, with_ties = F) |>
    ungroup() |>
    mutate(bases_allowed = case_when(result.eventType %in% c("single", "walk", "hit_by_pitch", "intent_walk") ~ 1,
                                     ...
                                     T ~ 0)) |>
    group_by(pitcher_name = matchup.pitcher.fullName) |>
    summarise(bases_allowed = sum(bases_allowed)) |>
    arrange(desc(bases_allowed))
  
  res = pitcher_outs_recorded |>
    inner_join(pitcher_bases_allowed, by = "pitcher_name")
  
  return(res)
}

# all_pitcher_bases_outs = data.frame()
# 
# for (new_pk in end_games$game_pk) {
#   all_pitcher_bases_outs = bind_rows(all_pitcher_bases_outs, get_game_pitcher_outs_bases(pk = new_pk))
# }
# 
