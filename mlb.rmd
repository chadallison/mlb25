...
if (truncate_blowouts) {
  end_games = end_games |>
    filter(win_score - lose_score <= max_margin)
}
```
...
team_npr |>
  mutate(pl = ifelse(ovr_npr >= 0, ovr_npr, ""),
         nl = ifelse(ovr_npr < 0, ovr_npr, "")) |>
  ...
  labs(x = NULL, y = "Overall NPR",
       title = glue("Team NPR standings as of {better_date(Sys.Date())}"))
```
...
home_wl |>
  ...
  labs(x = "Home Win Percentage", y = "Away Win Percentage",
       title = glue("Home and away win percentages as of {better_date(Sys.Date())}")) +
...
team_scored_allowed |>
  ...
  labs(x = "Window number", y = "Pythagorean win percentage",
       title = "Pythagorean win percentages in three-game rolling windows")
```
...
team_npr_by_game |>
  ...
  arrange(team, date) |>
  mutate(roll_npr = rollapply(npr, width = 3, align = "right", FUN = "mean", fill = NA), .by = "team") |>
  ...
  labs(x = "Window number", y = "NPR",
       title = "Team NPR in three-game rolling windows")
```
...
above_below_500_res |>
  ...
  labs(x = "Win Percentage vs. Above .500 Teams",
       ...
       title = glue("Win percentages vs. above/below .500 teams as of {better_date(Sys.Date())}")) +
  scale_x_continuous(breaks = seq(0, 100, by = 10), labels = scales::percent_format(scale = 1)) +
  scale_y_continuous(breaks = seq(0, 100, by = 10), labels = scales::percent_format(scale = 1))
```
...
```{r echo = F}
team_diffs = team_scored_allowed |>
  group_by(team) |>
  summarise(scored = sum(scored),
            allowed = sum(allowed)) |>
  mutate(diff = scored - allowed,
         pl = ifelse(diff >= 0, diff, ""),
         nl = ifelse(diff < 0, diff, "")) |>
  inner_join(teams_info, by = "team")

team_diffs |>
  ggplot(aes(reorder(team, diff), diff)) +
  geom_col(aes(fill = team), show.legend = F) +
  coord_flip(ylim = c(min(team_diffs$diff) * 1.05, max(team_diffs$diff) * 1.05)) +
  scale_fill_manual(values = team_hex) +
  scale_y_continuous(breaks = seq(-500, 500, by = 25)) +
  geom_text(aes(label = pl), size = 3, hjust = -0.25) +
  geom_text(aes(label = nl), size = 3, hjust = 1.25) +
  labs(x = NULL, y = "Run Differential",
       title = glue("Team run differentials as of {better_date(Sys.Date())}"))
```
...
end_py = end_games |>
  inner_join(team_pythag_small, by = c("home_team" = "team")) |>
  rename(home_pythag = pythag) |>
  inner_join(team_pythag_small, by = c("away_team" = "team")) |>
  rename(away_pythag = pythag)

home_opp_py = end_py |>
  select(date, team = home_team, opp_pythag = away_pythag)

away_opp_py = end_py |>
  select(date, team = away_team, opp_pythag = home_pythag)

team_opp_py_by_game = bind_rows(home_opp_py, away_opp_py) |>
  arrange(team, date)

team_opp_py = team_opp_py_by_game |>
  group_by(team) |>
  summarise(avg_opp_pythag = round(mean(opp_pythag), 3))

team_pythag_small |>
  inner_join(team_opp_py, by = "team") |>
  inner_join(teams_info, by = "team") |>
  ggplot(aes(avg_opp_pythag, pythag)) +
  geom_point(aes(col = team), shape = "square", size = 4, show.legend = F) +
  geom_vline(xintercept = 0.5, linetype = "dashed", alpha = 0.5) +
  geom_hline(yintercept = 0.5, linetype = "dashed", alpha = 0.5) +
  scale_color_manual(values = team_hex) +
  ...
  labs(x = "Avg. Opponent Pythagorean Win Percentage",
       ...
       title = glue("Performance by Strength of Schedule as of {better_date(Sys.Date())}")) +
